...
import { DEMO_PROPERTIES, DEMO_LANDLORDS } from '../../services/Propertyservice';
 ...
 *
 *   Property feed reads DEMO_PROPERTIES (single source of truth) — the same
 *   list the listings + details screens consume. There is no parallel mock.
 */
...
/** Convert a property `date` string ("2026-04-20") into "days ago" relative
 *  to today. We anchor on the newest DEMO_PROPERTIES date so the demo feed
 *  always shows fresh items regardless of the calendar — once the backend
 *  is wired this becomes `Date.now() - new Date(p.date)`. */
const referenceDate = (() => {
  const dates = DEMO_PROPERTIES.map((p) => new Date(p.date).getTime());
  return new Date(Math.max(...dates));
})();
const daysBetween = (a, b) => Math.max(0, Math.floor((b - a) / (1000 * 60 * 60 * 24)));
const computeDaysAgo = (dateStr) => daysBetween(new Date(dateStr).getTime(), referenceDate.getTime());
 ...
 */
const PopularAreasBento = ({ t, onPickArea }) => {
  // Build (area, featuredProperty) pairs from real DEMO_PROPERTIES so each
  // tile still has a nice cover image. Falls back to the first property
  // when no listing matches the area name yet.
  const tiles = useMemo(() => {
    return POPULAR_AREAS.map((area) => {
      const featured =
        DEMO_PROPERTIES.find((p) =>
          p.location.toLowerCase().includes(area.toLowerCase())
        ) || DEMO_PROPERTIES[0];
      const homeCount = DEMO_PROPERTIES.filter((p) =>
        p.location.toLowerCase().includes(area.toLowerCase())
      ).length;
      return { area, property: featured, homeCount };
    });
  }, []);
 ...
 */
const PropertyCard = ({ property, t }) => {
  const navigate = useNavigate();
  const [liked, setLiked] = useState(false);
  const { typeLabel, catLabel } = cardLabels(property, t);

  const daysAgo = computeDaysAgo(property.date);
  const ageLabel = daysAgo === 0
    ? t.mobNewToday
    : `${daysAgo} ${daysAgo === 1 ? t.mobDayAgo : t.mobDaysAgo}`;
  const landlord = DEMO_LANDLORDS[property.landlordId];

  // Up to 4 images: 1 primary + 3 thumbs. If the property has fewer,
  // recycle the primary so the layout stays consistent.
  const [primary, ...rest] = property.images;
  const thumbs = [...rest, primary, primary].slice(0, 3);
...
const MobileHome = () => {
  ...
  const filteredFeed = useMemo(() => {
    return DEMO_PROPERTIES
      .map((p) => ({ ...p, _daysAgo: computeDaysAgo(p.date) }))
      .filter((p) => {
        if (!activeArea) return true;
        return p.location.toLowerCase().includes(activeArea.toLowerCase());
      })
  ...
  return (
    <div className="md:hidden bg-slate-50 min-h-screen pb-28">
      ...
      <PopularAreasBento t={t} onPickArea={setOpenArea} />
      ...
      {filteredFeed.length === 0 ? (
        ...
        <>
          {filteredFeed.slice(0, 2).map((p) => (
            <PropertyCard key={p.id} property={p} t={t} />
          ))}
          <LandlordCTA t={t} />
